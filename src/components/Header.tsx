      ...
      <div className="w-full px-3 sm:px-6 lg:px-8 h-10 sm:h-11 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            {/* User Avatar / Profile Icon Dropdown Menu */}
            {(adminUser || currentUser) && (
              <div className="relative shrink-0 pl-1">
                ...
                >
                  {!adminUser && currentUser && (
                    <span className={`text-xs font-semibold whitespace-nowrap capitalize tracking-tight hidden xs:inline ${
                      isDark ? 'text-slate-200' : 'text-slate-700'
                    }`}>
                      {currentUser.full_name || currentUser.name}
                    </span>
                  )}
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 border shadow-xs ${
                    adminUser 
                  ...
                  <ChevronDown className={`w-3 h-3 transition-transform ${showUserDropdown ? 'rotate-180' : ''} ${
                    adminUser ? 'text-white/80' : (isDark ? 'text-slate-400' : 'text-slate-600')
                  }`} />
                </button>
                  ...
                  {showUserDropdown && (
                    <>
                      ...
                      <motion.div
                        ...
                        className={`absolute right-0 mt-2 w-48 rounded-xl border shadow-xl z-50 py-1.5 text-xs font-semibold overflow-hidden ${
                      ...
                      >
                        <div className="px-3 py-2 border-b border-slate-200/50 dark:border-slate-800">
                          <p className="font-bold text-slate-900 dark:text-white truncate">
                            {currentUser?.full_name || adminUser?.full_name || 'User'}
                          </p>
                          <p className="text-[10px] text-slate-400 capitalize truncate">
                            {adminUser ? (adminUser.role || 'Admin IT') : (currentUser?.jabatan || 'Karyawan')}
                          </p>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full text-left px-3 py-2 flex items-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          onClick={() => {
                            setShowUserDropdown(false);
                            if (onOpenMyAssets) onOpenMyAssets();
                          }}
                          className="w-full text-left px-3 py-2 flex items-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer"
                        >
                        ...
                        <div className="my-1 border-t border-slate-200/50 dark:border-slate-800" />
                        ...
                        <button
                          ...
                          className="w-full text-left px-3 py-2 flex items-center gap-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-all cursor-pointer"
                        >
                          <LogOut className="w-3.5 h-3.5" />
