...
export const ProjectEvaluation: React.FC<ProjectEvaluationProps> = ({
  ...
  const handleClearData = async () => {
    if (!selectedProjectId) return;
    if (window.confirm('Apakah Anda yakin ingin mengosongkan seluruh data mentah dari project ini?')) {
      try {
        setLoadingDashboard(true);
        await api.clearEvalProjectData(selectedProjectId);
        toast.success('Data project berhasil dikosongkan');
        fetchDashboard(selectedProjectId);
        fetchProjects();
      } catch (err) {
        console.error(err);
    ...
    try {
      ...
      if (template === 'm365') {
        for (let day = 14; day >= 0; day--) {
          ...
          const entriesCount = Math.floor(Math.random() * 8) + 4; // 4 to 11 entries
          for (let e = 0; e < entriesCount; e++) {
            const user = m365Users[Math.floor(Math.random() * m365Users.length)];
            const activity = m365Actions[Math.floor(Math.random() * m365Actions.length)];
  ...
  const rawRecords = useMemo(() => {
    ...
    // We can simulate fetching all rows or build from aggregated stats. Since the API returns trend/top users/etc, we can fetch all records. Let's make sure we have a search/filter on the raw records if we show them. Wait, let's look at how we display top users or trend.
    // If we want a detailed list, we can extract from topUsers, but let's see: we want the user to inspect and search imported data. Let's construct a list or keep records state.
    // Wait, the API returns raw stats, let's keep a simulated table of activity list if we didn't return all raw rows, or we can make sure the trend and department distribution represents the aggregated view beautifully.
    // Let's create mock detailed records if they imported them, or use the trend/topUsers to display active logs.
    // Wait, let's look at the structure of dashboardData. It returns `trend`, `departmentDistribution`, `activityDistribution`, `topUsers`. We can reconstruct a list of records for listing, or we can fetch a query if we want. But showing Top Active Users, Activity Distribution, and Trend is exactly what's required for a gorgeous high-fidelity dashboard! Let's display the table of "Top Pengguna" and "Distribusi Departemen" as main interactive items.
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                  <div className={cardClass}>
                    <div className="flex items-center justify-between mb-3">
                      <span className={`text-[10px] font-bold uppercase tracking-wider ${textMuted}`}>Total Penggunaan (Hits)</span>
                      <div className="w-7 h-7 rounded-lg bg-emerald-500/10 text-emerald-500 flex items-center justify-center">
                        <Activity className="w-3.5 h-3.5" />
                      </div>
                    </div>
                    <div className={`text-2xl sm:text-3xl font-black ${textMain}`}>
                      {dashboardData?.stats?.total_usage || 0}
                    </div>
                    <p className="text-[10px] text-emerald-500 font-bold mt-1">
                      {dashboardData?.stats?.total_records || 0} baris data raw
                    </p>
                  </div>
                  ...
                  <div className={cardClass}>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      Target user: {dashboardData?.stats?.target_users || 0}
                    </p>
                  </div>
                  ...
                  <div className={cardClass}>
                    ...
                    <div className={`text-2xl sm:text-3xl font-black ${textMain}`}>
                      {dashboardData?.stats?.active_departments || 0}
                    </div>
                    <p className="text-[10px] text-slate-400 mt-1">
                  ...
                  <div className={cardClass}>
                    ...
                    <div className={`text-2xl sm:text-3xl font-black ${textMain}`}>
                      {dashboardData?.stats?.adoption_rate || 0}%
                    </div>
                    <div className="w-full h-1 bg-slate-500/10 rounded-full mt-2 overflow-hidden">
                      <div 
                        className="h-full bg-yellow-500 rounded-full" 
                        style={{ width: `${Math.min(dashboardData?.stats?.adoption_rate || 0, 100)}%` }}
                      />
                    </div>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                      ...
                      <div className={`${cardClass} flex flex-col justify-between h-[360px]`}>
                          ...
                          <ResponsiveContainer width="100%" height="80%">
                            <PieChart>
                              <Pie
                                ...
                                nameKey="type"
                              >
                                {(dashboardData?.activityDistribution || []).map((entry: any, index: number) => (
                        ...
                        <div className="max-h-[100px] overflow-y-auto space-y-1 px-1">
                          {(dashboardData?.activityDistribution || []).slice(0, 4).map((entry: any, index: number) => (
                            <div key={entry.type} className="flex items-center justify-between text-[11px]">
                              ...
                              <span className={textMuted}>{entry.count} ({Math.round(entry.count / (dashboardData?.stats?.total_usage || 1) * 100)}%)</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      ...
                      <div className={`${cardClass} h-[380px] flex flex-col justify-between`}>
                        ...
                        <div className="flex-1 overflow-y-auto mt-4 pr-1 space-y-2">
                          {(dashboardData?.topUsers || []).slice(0, 10).map((user: any, index: number) => (
                            <div 
                              key={user.user_name}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                      ...
                      <div className={`${cardClass} space-y-4`}>
                        <div>
                          <h3 className={`text-sm font-black ${textMain}`}>Petunjuk & Generator Simulasi</h3>
                          <p className={`text-xs ${textMuted}`}>Klik salah satu template di bawah untuk menguji dashboard instant dengan data realistis.</p>
                        </div>
                        ...
                        <button
                          ...
                          disabled={dashboardData?.stats?.total_records === 0}
                    ...
                    {dashboardData?.stats?.total_records > 0 && (
                      <div className={cardClass}>
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                          <div>
                            <h3 className={`text-sm font-black ${textMain}`}>Ringkasan Penggunaan per User</h3>
                            <p className="text-[10px] text-slate-400">Total interaksi tiap anggota di database project ini</p>
                          </div>
                        ...
                        <div className="flex items-center justify-between pt-4 mt-2">
                          <span className="text-[10px] text-slate-400 font-bold">
                            Menampilkan {Math.min(itemsPerPage, rawRecords.length)} dari {rawRecords.length} user
                          </span>
                          <div className="flex gap-1">
                            ...
                            <button
                              disabled={currentPage * itemsPerPage >= rawRecords.length}
